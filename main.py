...
while cv.waitKey(1) < 0:
    ...
    if has_frame:
        ...
        # pose estimation  用opencv从摄像头中读取一帧一帧的图像数据，然后将图像数据传给TfPoseEstimator类的inference函数,得到一个返回值humans
        humans = estimator.inference(show)
        # get pose info  humans和图像数据image再传给TfPoseVisualizer类的draw_pose_rgb函数，得到的返回值通过opencv显示出来，就是我们看到的人体姿态检测图
